...
/** Inruilflow v3 — scherm 1: Wat verkoop je? (vlakke opzet, zie components/trade-in/v3/shared.tsx)
 *  Start = grote zoekbalk. Product gekozen → kaart met conditie + shuttercount (open, default "Tot 25.000") + "Voeg dit item toe".
 *  Eén primaire CTA (oranje, sticky onderaan): "Vraag je bod aan". */
...
import { useV3State, V3Header, Thumb, IconBtn, CONDITIONS, C, input, card, btnCta, btnGhost, type SellItem } from '@/components/trade-in/v3/shared';

const DEFAULT_SHUTTER = SHUTTER_RANGES[0].label; // "Tot 25.000"

export default function TradeInV3Page() {
    ...
    <>
      ...
      <div style={{ background: C.surface, padding: '36px 0 96px' }}>
        <div style={{ maxWidth: 880, margin: '0 auto', padding: '0 24px' }}>
          <h2 style={{ fontSize: 30, fontWeight: 800, letterSpacing: '-.02em', margin: 0, color: C.text }}>Wat verkoop je?</h2>
          <p style={{ color: C.sec, margin: '6px 0 22px', fontSize: 15 }}>Zoek je product, kies de conditie en vraag je bod aan.</p>
          ...
          {!picked && (visibleItems.length === 0 || searchOpen) && (
            <div style={{ position: 'relative', margin: visibleItems.length ? '14px 0 0' : 0 }}>
              <input
                ...
                autoComplete="off"
                style={{ ...input, borderRadius: 999, padding: '18px 60px 18px 24px', fontSize: 16, boxShadow: '0 2px 10px rgba(30,33,51,.06)' }}
              />
              ...
              {showResults && q.trim().length >= 2 && (
                <div style={{ position: 'absolute', left: 8, right: 8, top: 'calc(100% + 8px)', zIndex: 20, background: '#fff', borderRadius: 14, boxShadow: '0 12px 32px rgba(45,48,71,.16)', overflow: 'hidden' }}>
                  {results.length ? results.map((p, i) => (
                    <div key={p.name} onMouseDown={() => pick(p)} className="ti3-row" style={{ borderBottom: i < results.length - 1 ? `1px solid ${C.border}` : 'none' }}>
          ...
          {picked && (
            <div style={{ ...card, padding: 22, marginTop: visibleItems.length ? 14 : 0, marginBottom: 12, border: `1.5px solid ${C.accent}` }}>
              ...
              <div className="ti3-cond-grid">
                {CONDITIONS.map(c => {
                  const sel = condition === c.label;
                  return (
                    <button key={c.label} onClick={() => setCondition(c.label)} className="ti3-cond" style={{ borderColor: sel ? C.accent : C.border, background: sel ? C.accentSoft : '#fff', color: C.text, boxShadow: sel ? `inset 0 0 0 1px ${C.accent}` : 'none' }}>
              ...
              {showHelp && (
                <div style={{ marginTop: 12, border: `1px solid ${C.border}`, borderRadius: 12, padding: '4px 16px', background: '#FAFAFC' }}>
                  {CONDITIONS.map((c, i) => (
      ...
      {/* Sticky onderbalk met de enige primaire CTA */}
      <div style={{ position: 'sticky', bottom: 0, background: '#fff', borderTop: `1px solid ${C.border}`, padding: '14px 24px', zIndex: 15, boxShadow: '0 -6px 20px rgba(30,33,51,.06)' }}>
        <div style={{ maxWidth: 880, margin: '0 auto', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, flexWrap: 'wrap' }}>
          <div style={{ fontSize: 13.5, color: C.sec }}>
            {items.length ? <><strong style={{ color: C.text }}>{items.length} item{items.length > 1 ? 's' : ''}</strong> — geen verkoopkosten, gratis verzekerd verzenden.</> : 'Voeg minimaal één item toe om je bod aan te vragen.'}
          </div>
          <button disabled={!items.length} onClick={() => router.push('/trade-in/v3/gegevens')} style={{ ...btnCta, opacity: items.length ? 1 : 0.45, cursor: items.length ? 'pointer' : 'default', padding: '15px 30px' }}>
            Vraag je bod aan →
          </button>
        </div>
      </div>
      ...
      <style>{`
        .ti3-row{display:flex;align-items:center;gap:12px;padding:11px 16px;cursor:pointer}
        .ti3-row:hover{background:#FFFBF7}
        .ti3-cond-grid{display:grid;grid-template-columns:repeat(5,1fr);gap:8px}
        @media(max-width:760px){.ti3-cond-grid{grid-template-columns:repeat(2,1fr)}}
        .ti3-cond{border:1.5px solid #EEEEF2;border-radius:12px;padding:14px 8px;font-size:14px;cursor:pointer;font-family:inherit;transition:all .15s;text-align:center;line-height:1.2}
        .ti3-cond:hover{border-color:#E8692A}
      `}</style>
    </>
  );
}
