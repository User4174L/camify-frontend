  ...
  return (
    <>
      ...
      <Page width={680}>
        ...
        <PageTitle title="Je gegevens" />

        <div style={{ ...card, padding: 24 }}>
          <div className="tiw-form-grid">
            <label className="tiw-label"><span>Voornaam *</span><input value={c.firstName} onChange={e => set({ firstName: e.target.value })} onFocus={centerOnFocus} style={input} /></label>
            <label className="tiw-label"><span>Achternaam *</span><input value={c.lastName} onChange={e => set({ lastName: e.target.value })} onFocus={centerOnFocus} style={input} /></label>
            <label className="tiw-label"><span>E-mailadres *</span><input type="email" value={c.email} onChange={e => set({ email: e.target.value })} onFocus={centerOnFocus} style={input} placeholder="jij@voorbeeld.nl" /></label>
            <label className="tiw-label"><span>Telefoon <span style={{ color: C.sec, fontWeight: 500 }}>(optioneel)</span></span><input type="tel" value={c.phone} onChange={e => set({ phone: e.target.value })} onFocus={centerOnFocus} style={input} placeholder="06 12345678" /></label>
          </div>
            ...
            <div style={{ marginTop: 14 }}>
              <div className="tiw-form-grid">
                <label className="tiw-label"><span>Vestigingsland *</span>
                  <select value={c.country} onChange={e => set({ country: e.target.value })} onFocus={centerOnFocus} style={input}>
                    {COUNTRIES.map(([v, l]) => <option key={v} value={v}>{l}</option>)}
                  </select>
                </label>
                {!NON_EU.includes(c.country) && (
                  <label className="tiw-label"><span>BTW-nummer *</span>
                    <input value={c.vat} onChange={e => set({ vat: e.target.value })} onFocus={centerOnFocus} style={{ ...input, borderColor: c.vat && !vatOk ? '#dc2626' : C.border }} placeholder={c.country === 'NL' ? 'NL123456789B01' : `${c.country}…`} />
