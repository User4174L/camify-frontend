...
**Bron (data):** categories (multi, fase 1) + brands (optioneel), count, columns. **Filterknoppen:** repeater; per knop categorieën (multi) óf een product_type binnen de scope — één van beide is genoeg (zie comment #429). Eerste knop "Alles" automatisch. **Fase 2:** spec-filter als bron (sensor = full frame).
